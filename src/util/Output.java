...
public class Output {
    ...
    protected boolean flushAltijd = true;
    ...
    public Output(boolean flushAltijd) {
        this(System.out, flushAltijd);
    }
    
    public Output(String name, boolean flushAltijd) {        
        this.out = open(try_new_FileOutputStream(name));
        this.flushAltijd = flushAltijd;        
    }
    
    public Output(PrintStream out, boolean flushAltijd) {
        this.out = open(out);
        this.flushAltijd = flushAltijd;
    }
    ...
    public void println(short s, int lengte) {
        println(String.valueOf(s), lengte);
    }
    ...
    private String kommaNaarLinks(String s, int n) {
        int aantalTientallen, indexDecimalePunt; //, aantalDecimalen;
        String tientallen, decimalen, extraNullen;
    ...
    private String kommaNaarRechts(String s, int n) {
        ...
        if (n < aantalDecimalen) {
            tientallen += s.substring(indexDecimalePunt + 1, indexDecimalePunt
                    + 1 + n);
    ...
    private String afgerond(String s, int ndec) {
        int i, indexDecimalePunt, aantalTientallen, aantalDecimalen;
        String teken, extraNullen;
        ...
        if (ndec < aantalDecimalen) {
            ...
            if (s.charAt(indexDecimalePunt + ndec + 1) >= '5') {
                ...
                // dat geen '9' is.
                for (i = indexDecimalePunt - 1; i > -1 && s.charAt(i) == '9'; i--) {
                    // doe niets
                }

                if (i == -1) {
                    // het af te ronden getal bevat alleen negens.
                    s = "1" + nullen(aantalTientallen + ndec) + ".";
                } else if (i == 0) { // s.charAt(0) != '9'
